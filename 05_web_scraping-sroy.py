...
res=b.find_all(name='h2')
print "{} items".format(len(res))
for x in res:
    print x.text
    
# find the 'p' tag with an 'id' value of 'feedback' and then print its text
res=b.find_all(name='p', attrs={'id':'feedback'})
print "{} items".format(len(res))
for x in res:
    print x.text
...
f = codecs.open(r'mylocaldir/Shawshank.html', "w", 'utf-8')
f.write (b.prettify()) 
f.close()
...
res=b.find(attrs = {'class':'summary_text'})     # first try
print res

#                                                   second try:
res=b.find_all(attrs = {'itemprop':'description'})
print len(res)
for x in res:
    print x.text
...
res=b.find_all(attrs = {'itemprop':'contentRating'})     # first try
print len(res)
for x in res:
    print x.text

res=b.find(name='meta', attrs = {'itemprop':'contentRating'})     # second try
print res['content'] # oh, it's a dict.
    
# get the duration in minutes (as an integer)
res=b.find_all(name='time', attrs = {'itemprop' : 'duration'})
print len(res)
for x in res:
    print x.text
#not sure how to convert those to datetime
'''
...
# no error checking or anything going on, but here it is:
def get_movie_info(iid):
    rdict=dict()
#    print 'http://www.imdb.com/title/' + iid + '/'
    r = requests.get('http://www.imdb.com/title/' + iid + '/')
    b = BeautifulSoup(r.text)
    
    res=b.find(name='meta', attrs = {'itemprop':'contentRating'})     # second try
    rdict['content_rating']=res['content']

    res=b.find(attrs = {'class':'summary_text'})     # first try
    rdict['description'] = res.text.strip()
    
    res=b.find_all(name='time', attrs = {'itemprop' : 'duration'})
    rdict['duration']=int(re.sub('[^0-9]','', res[-1].text)) # oops, it's a string

    rdict['star_rating']=float(b.find(name='span', attrs={'itemprop':'ratingValue'}).text)
    
    res=b.find(name='div', attrs = { 'class' : 'title_block'}).find(attrs = {'itemprop' : 'name'})
    rdict['title']=res.text.replace(u'\xa0', u' ').strip()
    return rdict

# test the function
res=get_movie_info('tt0111161')

# open the file of IDs (one ID per row), and store the IDs in a list
with open('data/imdb_ids.txt', 'rU') as f:
    idf = f.readlines()
idf=[x.strip('\n') for x in idf]
...
b.find_all(name='span', attrs={'itemprop':'creator'})

# limit search to a smaller section to only get the writers
b.find(name='span', attrs={'itemprop':'creator'}).find_all(name='span', attrs={'itemprop':'name'})
